...
async function generarPdfCotizacion(q) {
  ...
  if (logoData) { try { doc.addImage(logoData, 'PNG', marginL, y, 46, 46) } catch (e) { /* logo opcional */ } }
  const headCenterX = marginL + 46 + (pageW - marginR - (marginL + 46)) / 2
  doc.setFont('times', 'bold'); doc.setFontSize(15); doc.setTextColor(...NAVY)
  doc.text(DATOS_EMPRESA.razon, headCenterX, y + 12, { align: 'center' })
  doc.setFont('times', 'normal'); doc.setFontSize(8); doc.setTextColor(30, 30, 30)
  doc.text(DATOS_EMPRESA.rubro, headCenterX, y + 26, { align: 'center' })
  doc.setFontSize(7); doc.setTextColor(...BLUE2)
  doc.text(`Rut: ${DATOS_EMPRESA.rut} / ${DATOS_EMPRESA.direccion}`, headCenterX, y + 38, { align: 'center' })
  doc.text(`Teléfono: ${DATOS_EMPRESA.telefono}    Web: ${DATOS_EMPRESA.web}    Correo: ${DATOS_EMPRESA.correo}`, headCenterX, y + 49, { align: 'center' })

  y += 72
  doc.setDrawColor(0); doc.setLineWidth(0.75); doc.line(marginL, y, pageW - marginR, y)
  y += 18

  doc.setFont('times', 'bold'); doc.setFontSize(8); doc.setTextColor(...NAVY)
  doc.text('Cliente:', marginL, y)
  doc.text(q.cliente || '—', marginL + 42, y)
  doc.setTextColor(0, 0, 0)
  doc.text(`N°.    ${String(q.numero).padStart(6, '0')}`, pageW - marginR - 130, y)
  y += 13
  doc.setFont('times', 'normal'); doc.setTextColor(0, 0, 0)
  if (q.cliente_rut) doc.text(q.cliente_rut, marginL + 42, y)
  doc.setFont('times', 'bold')
  doc.text(new Date(q.fecha + 'T00:00:00').toLocaleDateString('es-CL', { day: '2-digit', month: 'long', year: 'numeric' }).toUpperCase(), pageW - marginR - 130, y)
  doc.setFont('times', 'normal')
  y += 13
  if (q.cliente_direccion) { doc.text(q.cliente_direccion, marginL + 42, y); y += 13 }
  if (q.cliente_correo) { doc.text(q.cliente_correo, marginL + 42, y); y += 13 }
  ...
  autoTable(doc, {
    ...
    theme: 'plain',
    styles: { font: 'times', fontSize: 8, cellPadding: { top: 5, bottom: 5, left: 2, right: 2 }, lineColor: [90, 90, 90], lineWidth: { bottom: 0.5 } },
    headStyles: { font: 'times', fontStyle: 'bold', fontSize: 7.5, textColor: NAVY, lineColor: [0, 0, 0], lineWidth: { bottom: 0.75 } },
    columnStyles: {
      0: { cellWidth: 32, halign: 'center', fontStyle: 'bold', textColor: NAVY },
      1: { cellWidth: 'auto' },
  ...
  autoTable(doc, {
    ...
    body: totalRows,
    theme: 'plain',
    styles: { font: 'times', fontSize: 7.5, fontStyle: 'bold', cellPadding: 5, lineColor: [0, 0, 0], lineWidth: { bottom: 0.5 } },
  ...
  let by = doc.lastAutoTable.finalY + 16
  doc.setFont('times', 'bold'); doc.setFontSize(7.5); doc.setTextColor(...NAVY)
  const banco = ['Datos Bancarios:', ...DATOS_BANCARIOS]
  banco.forEach((line, i) => doc.text(line, marginL, by + i * 10.5))

  let cy = by + banco.length * 10.5 + 18
  function bloque(titulo, lineas) {
    doc.setFont('times', 'bold'); doc.setFontSize(7.5); doc.setTextColor(...NAVY)
    doc.text(titulo, marginL, cy); cy += 11
    doc.setFont('times', 'normal'); doc.setFontSize(7); doc.setTextColor(0, 0, 0)
    lineas.forEach(l => {
      const wrapped = doc.splitTextToSize('- ' + l, contentW)
      doc.text(wrapped, marginL, cy); cy += wrapped.length * 9 + 3
    })
    cy += 8
  }
...
function Cotizaciones({ cotizaciones, tarifasComunas, perfil, toast, reload }) {
  ...
  // Traslado: se calcula solo desde la tabla de Tarifas y se agrega como un ítem más (igual que en el ejemplo real)
  ...
  function agregarTraslado() {
    if (!trasladoComuna) { toast('Elige una comuna de destino'); return }
    const com = tarifasComunas.find(c => c.comuna === trasladoComuna)
    const valor = com ? (com['p' + trasladoTamano] || 0) : 0
    setItems(rows => [...rows, { descripcion: `Traslado a ${trasladoComuna} (ida y vuelta)`, unidad: 'un', cantidad: 1, valorUnit: valor }])
    toast('Traslado agregado como ítem — puedes ajustar cantidad o valor si es necesario')
  }
  ...
  return (
    <>
      ...
      <div className="card">
        ...
        <div className="section-sub" style={{margin:'16px 0 8px'}}>Traslado (opcional)</div>
        <div className="quick-row" style={{gridTemplateColumns:'1fr 1fr auto'}}>
          <div className="f-group"><label>Tamaño de camión (para calcular el valor)</label>
          ...
          <button className="btn-outline" onClick={agregarTraslado}>+ Agregar traslado como ítem</button>
        </div>
