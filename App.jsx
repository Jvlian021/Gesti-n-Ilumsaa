...
import {
  isoDate, startOfWeek, addDays, fmtMoney, DIAS, MESES,
...
function Sidebar({ perfil, view, setView }) {
  ...
  return (
    <aside className="sidebar">
      <div className="brand">
        <Mark />
        <div><div className="name">ILUMSA</div><div className="sub">ALZA HOMBRES</div></div>
      </div>
...
// ============================================================
function Dashboard({ perfil, camiones, reservas, cotizaciones, tarifaArriendo, tarifasComunas, calWeekStart, setCalWeekStart, setView, toast }) {
  ...
  const days = [...Array(7)].map((_, i) => addDays(calWeekStart, i))
          ...
          <div className="card">
            <div className="card-head">
              ...
              <div className="week-nav">
                ...
                <button onClick={() => setCalWeekStart(addDays(calWeekStart, 7))}>›</button>
                <button className="today-btn" onClick={() => setCalWeekStart(startOfWeek(new Date()))}>Hoy</button>
              </div>
            </div>
                  ...
                  {camiones.map(c => (
                    <tr key={c.id}>
                      ...
                      {days.map(d => {
                        ...
                        if (est === 'Mantención') {
                          return <td key={+d}><div className={`badge small-info ${cls}`}>En Mantención<span className="b-sub">{c.hasta ? 'Hasta ' + c.hasta.slice(8,10)+'/'+c.hasta.slice(5,7) : ''}</span></div></td>
                        }
                        return <td key={+d}><div className={`badge ${cls}`}>{est}</div></td>
                      })}
                    </tr>
                  ))}
                </tbody>
...
function Camiones({ camiones, isAdmin, toast, reload }) {
  ...
  async function updateEstado(id, val) {
    const patch = { estado_general: val }
    if (val === 'Operativo') patch.hasta = null
    const { error } = await supabase.from('camiones').update(patch).eq('id', id)
    if (error) toast('No se pudo actualizar (revisa tus permisos)')
    else { toast('Estado actualizado'); reload() }
  }
  async function updateHasta(id, val) {
    const { error } = await supabase.from('camiones').update({ hasta: val || null }).eq('id', id)
    if (!error) reload()
  }
  ...
  return (
    <>
      <div className="toolbar">
        <div><h1>Camiones</h1><div className="section-sub">Estado y datos de la flota</div></div>
        {isAdmin && <button className="btn-orange" onClick={() => setShowModal(true)}>+ Nuevo camión</button>}
      </div>
      <div className="card" style={{padding:0}}>
        <table className="data">
          <thead><tr><th>Camión</th><th>Patente</th><th>Tamaño</th><th>Aislado</th><th>Estado</th>{isAdmin && <th>Hasta</th>}</tr></thead>
          <tbody>
            {camiones.map(c => (
              <tr key={c.id}>
                ...
                <td>
                  {isAdmin ? (
                    <select value={c.estado_general} onChange={e => updateEstado(c.id, e.target.value)}>
                ...
                {isAdmin && <td><input type="date" value={c.hasta || ''} onChange={e => updateHasta(c.id, e.target.value)} style={{width:135}} /></td>}
              </tr>
            ))}
          </tbody>
        </table>
...
// ============================================================
function Reservas({ camiones, reservas, tarifasComunas, perfil, toast, reload }) {
  const [showModal, setShowModal] = useState(false)
  const today = isoDate(new Date())
  const [form, setForm] = useState({ camionId: '', cliente: '', fecha: today, comuna: tarifasComunas[0]?.comuna || '', direccion: '', estado: 'Reservado' })

  useEffect(() => { if (!form.camionId && camiones[0]) setForm(f => ({...f, camionId: camiones[0].id, comuna: f.comuna || tarifasComunas[0]?.comuna || ''})) }, [camiones, tarifasComunas]) // eslint-disable-line
  ...
  async function save() {
    ...
    setShowModal(false)
    setForm({ camionId: camiones[0]?.id || '', cliente: '', fecha: today, comuna: tarifasComunas[0]?.comuna || '', direccion: '', estado: 'Reservado' })
    reload(); toast('Reserva guardada')
  }
  ...
  return (
    <>
      <div className="toolbar">
        <div><h1>Reservas</h1><div className="section-sub">Agenda de servicios por camión</div></div>
        <button className="btn-orange" onClick={() => setShowModal(true)}>+ Nueva reserva</button>
      </div>
      <div className="card" style={{padding:0}}>
        <table className="data">
      ...
      {showModal && (
        <div className="modal-bg show">
          <div className="modal">
            <h3>Nueva reserva</h3>
            <div className="msub">Elige un camión disponible en la fecha indicada.</div>
